...
**这份 DROID 导出固定为 15 fps。** 数据时间直接使用 `frame_index / 15`；反向片段的时间也相应递减。加载器不读取相机时间戳来估计帧率。Stage2 的动作条件只使用序号，关闭 TCP 速度损失，未来 16 个动作不绑定执行频率；Stage1 的时间和速度监督不变。
...
| Stage2 后续历史帧用于动作条件的 TCP 投影越界    | 对应局部视觉特征替换为表示缺失的`missing_token`，保留序号和有效性信息。有效性依据当前使用的真值轨迹或模型恢复轨迹判断。                |
...
当前帧序号 0 及独立可学习类型 embedding。三类类型 ID 为 TCP 历史 0、未来动作 1、全局视觉 2。
